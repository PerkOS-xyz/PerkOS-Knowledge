...
export default function AdminClient() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!c?.ok && !b?.ok) setMsg('Admin access denied — connect an allowlisted wallet.');
  }, [address, adminFetch]);
  ...
  const priceOf = (tier: string) => cfg?.policies.find((p) => p.tier === tier)?.price;
  ...
  return (
    <main className="dashShell">
      ...
      {/* Pricing (read-only — env-config) */}
      <section className="metricsGrid">
        <article className="metric"><span>Billing mode</span><strong>{cfg?.mode ?? '…'}</strong></article>
        <article className="metric"><span>Public</span><strong>{priceOf('public')?.amount ?? '…'} {priceOf('public')?.currency ?? ''}</strong></article>
        <article className="metric"><span>Premium</span><strong>{priceOf('premium')?.amount ?? '…'} {priceOf('premium')?.currency ?? ''}</strong></article>
        <article className="metric"><span>Private</span><strong>{priceOf('private')?.amount ?? '…'} {priceOf('private')?.currency ?? ''}</strong></article>
      </section>
      <section className="dashPanel wide">
        <p className="eyebrow">Pricing</p>
        <p className="body">Prices + token/pay-to are env-config (<code>KNOWLEDGE_X402_*</code>). Token: {cfg?.env?.KNOWLEDGE_X402_TOKEN ?? '…'} · Pay-to: {cfg?.env?.KNOWLEDGE_X402_PAY_TO ?? '…'}. Change in <code>.env</code> + redeploy.</p>
      </section>

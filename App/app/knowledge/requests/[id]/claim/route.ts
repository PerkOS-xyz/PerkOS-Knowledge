  ...
  const response = await withDb(async (client) => {
    ...
    const result = await client.query(
      `UPDATE knowledge_requests
       SET status = 'claimed', claimed_by_agent_id = $2, claimed_at = coalesce(claimed_at, now()), updated_at = now()
    ...
    if (!result.rowCount) {
      ...
      return { error: 'request_not_open', status: 409, row: existing.rows[0] } as const;
    }
